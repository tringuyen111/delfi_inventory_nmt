...
const OrganizationPage: React.FC = () => {
    ...
    // FIX: Update the type of `orgToSave` to match the data sent from the form, which excludes `has_active_docs`.
    const handleSave = (orgToSave: Omit<Organization, 'id' | 'org_code' | 'updated_at' | 'has_active_docs'>): Organization => {
        ...
        if (modalState.mode === 'edit' && modalState.org) {
            ...
            setToastInfo({ message: 'Cập nhật Organization thành công', type: 'success' });
        } else { // create mode
            const now = new Date();
            ...
            setToastInfo({ message: 'Tạo Organization thành công', type: 'success' });
        }
        return savedOrg;
    };
    
    // FIX: Update the type of `orgToSave` to align with the `handleSave` function.
    ...
    const columns: Column<Organization>[] = useMemo(() => [
        { key: 'org_code', header: 'Mã Tổ chức' },
        { key: 'org_name', header: 'Tên Tổ chức' },
        { key: 'phone', header: 'Số điện thoại' },
        { key: 'email', header: 'Email' },
        { key: 'status', header: 'Trạng thái', render: (org) => (
        ...
        { key: 'updated_at', header: 'Cập nhật', render: (org) => new Date(org.updated_at).toLocaleDateString() },
        ...
        <div className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
            ...
            {filteredOrgs.length === 0 && !isLoading && (
                <div className="text-center py-16">
                    <h3 className="text-lg font-medium text-gray-800 dark:text-gray-100">Chưa có Organization</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Nhấn Create để thêm bản ghi đầu tiên.</p>
                </div>
            )}
